...
本路由配置使用 Vue 3 的 `defineAsyncComponent` 来优化大型组件的懒加载，并通过路由过渡动画和预加载机制提供更好的用户体验和性能。

## 最新优化（2026-03）

### 1. 路由过渡动画
- 为 AppMobile.vue 和 AppDesktop.vue 添加了平滑的页面切换过渡效果
- 使用 `page-fade` 过渡动画，避免页面切换时的突兀感
- 添加加载占位符，解决异步组件加载时的空白页面问题

### 2. 异步组件加载优化
- 将所有大型组件的 `delay` 设置为 0，立即显示加载状态
- 避免页面切换时的空白闪烁，提升用户体验
- 保持合理的超时时间，确保加载失败时能够正确处理

### 3. 路由预加载
- 使用 `requestIdleCallback` 在浏览器空闲时预加载常用页面
- 预加载首页、搜索、个人中心等高频访问页面
- 降级方案确保在不支持的浏览器中也能正常工作

## 文件结构

```
src/router/
├── index.js              # 路由主配置文件（含预加载逻辑）
├── asyncComponents.js   # 异步组件加载配置（优化后）
├── components/          # 路由相关组件
...
- `delay`: 延迟显示加载组件的时间（毫秒），优化后设置为 0ms 以立即显示加载状态
...
2. **零延迟加载**: 所有组件设置 delay=0，立即显示加载状态，避免空白闪烁
3. **路由预加载**: 在浏览器空闲时预加载常用页面，提升后续访问速度
4. **平滑过渡**: 页面切换使用淡入淡出动画，提升视觉体验
5. **错误恢复**: 自动重试机制提高加载成功率
6. **加载反馈**: 清晰的加载状态提示提升用户体验
...
1. ~~**预加载**: 在用户可能访问的页面进行预加载~~ ✅ 已完成
2. **智能预加载**: 根据用户行为预测并预加载可能访问的页面
3. **资源优先级**: 根据页面重要性设置不同的加载优先级
4. **缓存策略**: 对已加载的组件进行缓存（Vue Router 已内置）
5. **性能监控**: 添加加载时间监控，持续优化加载性能

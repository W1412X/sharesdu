...
export function useAuthorRestore(userId) {
   ...
   */
  const getStorageKey = () => {
    return `authorScanMsg|${userId.value || ''}`;
  };
  ...
  const restoreState = () => {
    try {
      ...
      return {
        itemType: scanMsg.itemType || 'article',
  ...
  const saveState = (state) => {
    try {
      ...
      const stateToSave = {
        itemType: state.itemType || 'article',
   ...
   * @returns {Number} 目标页码
   */
  const getTargetPageNum = (savedPageNum, contentType) => {
    if (!savedPageNum || !savedPageNum[contentType]) {
      return 1;
    }
    return savedPageNum[contentType];
  };

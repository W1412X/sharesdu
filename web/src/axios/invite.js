...
export const createInvitationCode = async (capacity, expiresDays) => {
    try {
        ...
        const response = await getaxiosInstance().post('/admin/invitation-codes', requestData);
...
export const getInvitationCodeList = async () => {
    try {
        await waitForLock('token');
        const response = await getaxiosInstance().get('/admin/invitation-codes');
        return response.data;
    } catch (error) {
        let dealResult = await dealAxiosError(error);
        if (dealResult.status == 1412) {
...
export const updateInvitationCodeStatus = async (code, isActive) => {
    try {
        ...
        const response = await getaxiosInstance().patch('/admin/invitation-codes', requestData);
        return response.data;
    } catch (error) {
        let dealResult = await dealAxiosError(error);
        if (dealResult.status == 1412) {
